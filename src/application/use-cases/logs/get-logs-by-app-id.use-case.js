...
class GetLogsByAppIdUseCase {
  constructor(logRepository) {
    ...
    // Validate that the repository implements the required contract interface
    if (typeof logRepository.findBy !== 'function') {
      throw new Error('LogRepository must implement the findBy() method from LogRepositoryContract');
    }
  ...
  async execute(appId, limit = 1000) {
    try {
      ...
      // Query logs via repository contract with timing
