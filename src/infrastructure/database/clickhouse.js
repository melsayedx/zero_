...
function createClickHouseClient() {
  const client = createClient({
    ...
    max_open_connections: 5,       // Maximum concurrent connections
    max_idle_connections: 2,        // Keep 50 connections ready (50% of max)
    keep_alive: {
      enabled: true,
    ...
    clickhouse_settings: {
      ...
      async_insert_threads: 2,                     // Parallel async insert threads

      // === Write Performance Settings ===
      max_insert_threads: 2,                        // Parallel insert execution
